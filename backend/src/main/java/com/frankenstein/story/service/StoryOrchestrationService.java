...
public class StoryOrchestrationService {
   ...
   public StoryOrchestrationService(StoryGenerationService storyGenerationService, ImageGenerationService imageGenerationService, AudioGenerationService audioGenerationService, FileStorageService fileStorageService, ProgressNotificationService progressService) {
   ...
   public String initiateStoryGeneration(StoryInput input) {
      ...
      Story story = Story.builder().id(storyId).input(input).status(StoryStatus.PENDING).createdAt(LocalDateTime.now()).build();
   ...
   private CompletableFuture<List<byte[]>> generateAllImages(String storyId, StoryStructure structure) {
      ...
      for (int i = 0; i < structure.getPages().size(); i++) {
         ...
         CompletableFuture<byte[]> imageFuture = imageGenerationService.generateImageWithRetry(page.getImagePrompt(), structure.getImageSeed() + i, 3)
                                                                       .thenApply(imageData -> {
   ...
   private CompletableFuture<List<AudioSet>> generateAllAudio(String storyId, StoryStructure structure) {
      ...
      for (int i = 0; i < structure.getPages().size(); i++) {
         ...
         CompletableFuture<AudioSet> audioFuture = CompletableFuture.supplyAsync(() -> {
            ...
            List<byte[]> effects = page.getSoundEffects().stream().map(effectName -> {
      ...
      return CompletableFuture.allOf(audioFutures.toArray(new CompletableFuture[0]))
                              .thenApply(v -> audioFutures.stream().map(CompletableFuture::join).collect(Collectors.toList()));
   }

   private void assembleStory(Story story, StoryStructure structure, List<byte[]> images, List<AudioSet> audioSets) {
      ...
      for (int i = 0; i < structure.getPages().size(); i++) {
         ...
         List<String> effectUrls = pageStructure.getSoundEffects()
                                                .stream()
