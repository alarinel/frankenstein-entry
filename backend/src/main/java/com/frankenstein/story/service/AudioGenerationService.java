...
public class AudioGenerationService {
   ...
   public AudioGenerationService(OkHttpClient httpClient, ObjectMapper objectMapper,
   ...
   public CompletableFuture<byte[]> generateNarration(String text) {
      return CompletableFuture.supplyAsync(() -> {
         try {
            ...
            requestBody.put("voice_settings", Map.of("stability", stability, "similarity_boost", similarityBoost));

            RequestBody body = RequestBody.create(objectMapper.writeValueAsString(requestBody), MediaType.parse("application/json"));

            String url = String.format("%s/text-to-speech/%s", apiUrl, voiceId);

            Request request = new Request.Builder().url(url).addHeader("xi-api-key", apiKey).addHeader("Accept", "audio/mpeg").post(body).build();
   ...
   public CompletableFuture<byte[]> generateSoundEffect(String effectDescription) {
      return CompletableFuture.supplyAsync(() -> {
         try {
            ...
            requestBody.put("voice_settings", Map.of("stability", 0.3, "similarity_boost", 0.5));
