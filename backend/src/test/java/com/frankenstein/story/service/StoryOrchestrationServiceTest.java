...
class StoryOrchestrationServiceTest {
    ...
    @BeforeEach
    void setUp() {
        service = new StoryOrchestrationService(
    ...
    void generateCompleteStory_Success() throws Exception {
        ...
        StoryStructure storyStructure = StoryStructure.builder()
        ...
        when(imageGenerationService.generateImage(anyString(), anyInt(), anyLong()))
                .thenReturn(CompletableFuture.completedFuture("images/page-1.png"))
                .thenReturn(CompletableFuture.completedFuture("images/page-2.png"));
        when(audioGenerationService.generateNarration(anyString(), anyInt()))
                .thenReturn(CompletableFuture.completedFuture("audio/narration-1.mp3"))
                .thenReturn(CompletableFuture.completedFuture("audio/narration-2.mp3"));
        when(audioGenerationService.generateAllSoundEffects(anyList(), anyInt()))
                .thenReturn(CompletableFuture.completedFuture(Arrays.asList("audio/effect-1.mp3")))
                .thenReturn(CompletableFuture.completedFuture(Arrays.asList("audio/effect-2.mp3")));
        when(audioGenerationService.estimateAudioDuration(anyString()))
                .thenReturn(5.0);
    ...
    void generateCompleteStory_SendsProgressUpdates() throws Exception {
        ...
        when(storyGenerationService.generateStory(input)).thenReturn(storyStructure);
        when(imageGenerationService.generateImage(anyString(), anyInt(), anyLong()))
                .thenReturn(CompletableFuture.completedFuture("images/test.png"));
        when(audioGenerationService.generateNarration(anyString(), anyInt()))
                .thenReturn(CompletableFuture.completedFuture("audio/test.mp3"));
        when(audioGenerationService.generateAllSoundEffects(anyList(), anyInt()))
                .thenReturn(CompletableFuture.completedFuture(Arrays.asList("audio/effect.mp3")));
        ...
        verify(progressNotificationService).sendProgress(eq("test-id"), argThat(progress ->
                progress.getStatus() == StoryStatus.GENERATING_STORY
        ));
        verify(progressNotificationService).sendProgress(eq("test-id"), argThat(progress ->
                progress.getStatus() == StoryStatus.GENERATING_IMAGES
        ));
        verify(progressNotificationService).sendProgress(eq("test-id"), argThat(progress ->
                progress.getStatus() == StoryStatus.GENERATING_AUDIO
        ));
        verify(progressNotificationService).sendProgress(eq("test-id"), argThat(progress ->
                progress.getStatus() == StoryStatus.ASSEMBLING
        ));
        verify(progressNotificationService).sendProgress(eq("test-id"), argThat(progress ->
                progress.getStatus() == StoryStatus.COMPLETED
        ));
    }
    ...
    void generateCompleteStory_WithStoryGenerationFailure_SendsFailedStatus() {
        ...
        when(storyGenerationService.generateStory(input))
                .thenThrow(new RuntimeException("Story generation failed"));
        ...
        verify(progressNotificationService).sendProgress(eq(storyId), argThat(progress ->
                progress.getStatus() == StoryStatus.FAILED
        ));
    }
    ...
    void generateCompleteStory_WithImageGenerationFailure_SendsFailedStatus() throws Exception {
        ...
        when(storyGenerationService.generateStory(input)).thenReturn(storyStructure);
        when(imageGenerationService.generateImage(anyString(), anyInt(), anyLong()))
                .thenReturn(CompletableFuture.failedFuture(new RuntimeException("Image generation failed")));
    ...
    void generateCompleteStory_CalculatesMetadataCorrectly() throws Exception {
        ...
        List<StoryStructure.PageStructure> pages = Arrays.asList(
                createTestPage(1),
                createTestPage(2),
                createTestPage(3)
        );
